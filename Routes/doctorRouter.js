...
// View Route for a single Doctor Profile (Must be placed before API route for /:id)
DrRouter.get('/:id', async (req, res, next) => {
    try {
        ...
        const currentPage = 'doctorProfile'; // Set current page for navigation highlighting
        const footerLinks = [
            { url: "/", text: "Home" },
            { url: "/User/about", text: "About Us" },
            { url: "/Department/view", text: "Departments" },
...
// API Routes (moved below view routes to ensure view rendering for /:id is prioritized)
...
DrRouter.get("/:id", DocController.getById); // This is the API route for getting doctor by ID
